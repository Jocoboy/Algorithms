...
/**
 * Heap tree where a node's key is always lower than or equal to its children's
 * and higher than or equal to its parent's.
 */
public class MinHeap implements Heap {
    ...
    private void swap(int index1, int index2) {
        HeapElement tempElement = minHeap.get(index1 - 1);
        minHeap.set(index1 - 1, minHeap.get(index2 - 1));
        minHeap.set(index2 - 1, tempElement);
    }
    ...
    /**
     * Toggle an element up to its right place as long as its key is lower than its
     * parent's.
     * 
    ...
    /**
     * Toggle an element down to its right place as long as its key is higher than
     * any of its children's.
     * 
     * @param index - target element index
     */
    private void toggleDown(int index) {
        boolean isWrongOrder = getElementKey(index) > getElementKey(Math.min(index << 1, minHeap.size()));
        while ((index << 1) <= minHeap.size() && isWrongOrder) {
            // Here we swap it with the child for lower key.
            if ((index << 1) < minHeap.size() && (getElementKey((index << 1) + 1) < getElementKey(index << 1))) {
            ...
            isWrongOrder = getElementKey(index) > getElementKey(Math.min(index << 1, minHeap.size()));
        }
    }
    ...
    public void deleteElement(int index) {
        ...
        minHeap.set(index - 1, getElement(minHeap.size()));
        minHeap.remove(minHeap.size() - 1);

        if (getElementKey(index) < getElementKey((int) Math.floor(index >> 1))) {
            toggleUp(index);
        } else if (((index << 1) <= minHeap.size()) && (getElementKey(index) > getElementKey(index << 1))) {
            toggleDown(index);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        MinHeap minHeap = new MinHeap(listElements);
        System.out.println("Original Heap.");
        for (int i = 1; i <= minHeap.getSize(); i++) {
            System.out.println(minHeap.getElement(i).getKey()); // 1.0 2.0 3.0 7.0 19.0 100.0 25.0 36.0 17.0
        }
        minHeap.deleteElement(3);
        System.out.println("After deletions...");
        for (int i = 1; i <= minHeap.getSize(); i++) {
            System.out.println(minHeap.getElementKey(i));// 1.0 2.0 17.0 7.0 19.0 100.0 25.0 36.0
        }
    }
}